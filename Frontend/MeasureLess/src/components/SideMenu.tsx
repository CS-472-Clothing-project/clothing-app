...
import { FaUserCircle, FaSignInAlt, FaSignOutAlt, FaHistory, FaRulerHorizontal } from "react-icons/fa";
import { useAuth } from "../contexts/authContext";
import { doSignOut } from "../firebase/auth";
import logo from "../assets/measurelesslogo.png";
  ...
  return (
    <>
      <header className="flex items-center justify-between bg-white/30 backdrop-blur-md p-3 rounded-2xl sticky top-2 z-20 shadow-sm border border-white/40 mb-6">
        ...
        {/* Left: Logo (Links to Get Measured / UserInput) */}
        <div className="flex items-center gap-3">
             <Link to="/userInput">
                <img src={logo} alt="MeasureLess Logo" className="h-10 md:h-12 w-auto object-contain hover:opacity-90 transition-opacity" />
             </Link>
            
            {title && (
                <h1 className="text-lg font-bold text-gray-800 ml-2 hidden md:block">{title}</h1>
            )}
        </div>
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 overflow-hidden ring-1 ring-black/5">
                    ...
                    <Link 
                      ...
                      className="flex items-center gap-3 px-5 py-3 hover:bg-indigo-50 text-indigo-700 font-semibold transition-colors border-b border-gray-50"
                    >
